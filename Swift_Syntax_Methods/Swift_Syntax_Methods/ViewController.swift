...
/**
* 1. 参数名由外部参数名和局部参数名组合, 中间用空格分隔. (局部参数名是用于方法内部使用, 外部参数名是用于方法调用时使用)
* 2. 使用一些介词指向第一个参数, 例如: with, for, by. 使用介词是为了方法在调用的时候能像句子一样被读取.
* 3. 默认情况下给第一个参数本地参数名, 给第二个及以后的参数本地和外部参数名
* 4. 在有些情况下, 可以给第一个参数一个外部参数名.
* 5. 可以使用_(下划线)作为参数的一个明确的外部参数名.(可以用_代替外部参数名)
* 详细说明请见Swift_Syntax_Functions
*/
